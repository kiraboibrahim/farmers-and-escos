...
export class StorageService {
  ...
  constructor(private configService: ConfigService) {
    ...
    this.uploadUrl = this.configService.get<string>('UPLOAD_URL');
  }

  async upload(file: Express.Multer.File, resource: Resource, id: any) {
  ...
  async uploadBatch(files: Express.Multer.File[], resource: Resource, id: any) {
    return await Promise.all(
      files.map(async (file) =>
        !!file ? await this.upload(file, resource, id) : undefined,
      ),
    );
  }
